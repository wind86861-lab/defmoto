...
import { formatUzPhone, digitsOnly } from '@/lib/phoneInput';
        ...
        <div className="space-y-3 rounded-3xl border border-brand-surface-border bg-brand-surface p-5">
          {mode === 'forgot' ? (
            forgotStep === 'phone' ? (
              <>
                <Field icon={Phone}>
                  <input {...inp} type="tel" inputMode="tel" maxLength={17} placeholder="+998 90 000 00 00" value={phone} onChange={(e) => setPhone(formatUzPhone(e.target.value))} />
                </Field>
